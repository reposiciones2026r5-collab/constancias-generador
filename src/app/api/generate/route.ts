...
export async function POST(request: Request) {
  try {
    ...
    // Cargar la imagen plantilla local (.jpg o .png) de forma 100% compatible con Edge Runtime
    ...
    if (imgRes.ok) {
      ...
      if (!jpgRes.ok) {
        throw new Error('No se pudo cargar la plantilla de imagen desde /public');
      }
